...
const Sidebar = () => {
  const sidebarItems = [
    ...
    },
    {
      label: "My Content",
      path: "/content",
      icon: <AiFillVideoCamera size={23} />,
      showOnMobile: false,
    },
    {
      label: "Collections",
      path: "/collections",
      icon: <AiFillBoxPlot size={23} />,
      showOnMobile: true,
    },
    {
      label: "Subscribers",
      path: "/subscribers",
      icon: <AiOutlineUserAdd size={23} />,
      showOnMobile: true,
    },
  ];
    ...
    <>
      <aside class="group fixed inset-x-0 bottom-0 z-40 w-full shrink-0 border-t border-white bg-[#121212] px-2 py-2 sm:absolute sm:inset-y-0 sm:max-w-[70px] sm:border-r sm:border-t-0 sm:py-6 sm:hover:max-w-[250px] lg:sticky lg:max-w-[250px]">
        <ul class="flex justify-around gap-y-2 sm:sticky sm:top-[106px] sm:min-h-[calc(100vh-130px)] sm:flex-col">
          {sidebarItems.map(({ label, path, icon, showOnMobile }) => {
            return (
              <li
                key={label}
                class={`${showOnMobile ? "" : "sm:block hidden"}`}
              >
                <Link
                  to={path}
