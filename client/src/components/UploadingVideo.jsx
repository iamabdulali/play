...
const UploadingVideo = ({
  ...
  return (
    <div class="absolute text-white inset-x-0 top-0 z-10 flex h-[calc(100vh-66px)] items-center justify-center bg-black/50 px-4 pb-[86px] pt-4 sm:h-[calc(100vh-82px)] sm:px-14 sm:py-8">
      <div class="w-full max-w-lg overflow-auto rounded-lg border border-gray-700 bg-[#121212] p-4">
        <div class="mb-4 flex items-start justify-between">
          <h2 class="text-xl font-semibold">
            {hasRequestCompleted ? "Uploaded Video" : "Uploading Video..."}
            <span class="block text-sm text-gray-300">
              Track your video uploading process.
            </span>
          </h2>
          <button class="h-6 w-6" onClick={() => setIsOpen(false)}>
            <svg
        ...
        <div class="mb-6 flex gap-x-2 border p-3">
          ...
          <div class="flex flex-col">
            ...
            <div class="mt-2 flex items-center">
              ...
              {hasRequestCompleted ? "Uploaded Successfully" : "Uploading..."}
            </div>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <button class="border px-4 py-3" onClick={() => setIsOpen(false)}>
            Cancel
          </button>
          <button
            onClick={() => setIsOpen(false)}
            class="bg-[#ae7aff] px-4 py-3 text-black disabled:bg-[#E4D3FF]"
            disabled={!hasRequestCompleted}
          >
            Finish
          </button>
        </div>
